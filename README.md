...
data/                   — Per-competition files live here: raw CSVs + .npy arrays
                          + generated submission CSVs. Everything for one
                          competition is colocated under a single directory.
```
...
- **One directory per competition.** Raw CSVs, preprocessed arrays, and generated submissions all live under `data/`. Switching competitions is `scout/bootstrap.py <slug>` — everything in `data/` belongs to the active competition.
...
- **Automatic submissions.** Every time `val_loss` improves, a Kaggle-ready submission CSV is generated under `data/` alongside the raw competition files.
...
This wipes `data/` of the old competition's files, pulls the new competition's data, resets the memory, and re-renders `AGENTS.md`. Your code in `train.py` and `prepare.py` is regenerated from the starter template **only if you delete them first** — otherwise bootstrap preserves your current code. Git history keeps both competitions' progress.
